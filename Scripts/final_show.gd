...
func _ready() -> void:
	...
	Rhythm.reset()
	$"CatSprite Bass".play("Idle "+Global.cat_color)
	$"CatSprite Drums".play("Idle "+Global.Drum_color)
	$"CatSprite Guitar".play("Idle "+Global.Guitar_color)
...
func action_selected(Action :String):
	...
	if Action == "Attack ":
		...
		heal = true
	load_song("res://Assets/Tracks/Main Song.JSON")
	audioplayer.stream = load("res://Assets/Tracks/Final Track.mp3")
...
func results_screen():
	...
	$"CanvasLayer/Results/MarginContainer/PanelContainer/HBoxContainer/VBoxContainer/Label".text = str(Rhythm.score)+" Score"
	$"CanvasLayer/Results/MarginContainer/PanelContainer/HBoxContainer/VBoxContainer/Label6".text = str(accuracy*100)+"%"
